...
def calc_transm(wave_grid, H_layer, H_base, U_layer, P_layer,P_base, T_layer, VMR_layer,
    ...
    k_cia, ID, cia_nu_grid, cia_T_grid, dH, mmw, Ptop, power, ray_on=True):
    """
...
def calc_weighting(wave_grid, U_layer, P_layer, T_layer, VMR_layer,
    k_gas_w_g_p_t, P_grid, T_grid, del_g, ScalingFactor,
    k_cia, ID, cia_nu_grid, cia_T_grid, dH):
    """
    Calculate emission spectrum using the correlated-k method.
    ...
    # Normalise
    for iwave in range(NWAVE):
        weighting_l_w[:,iwave] = weighting_l_w[:,iwave]/np.sum(weighting_l_w[:,iwave])
...
@jit(nopython=True)
def calc_contribution(wave_grid, U_layer, P_layer, T_layer, VMR_layer,
    k_gas_w_g_p_t, P_grid, T_grid, del_g, ScalingFactor,
    k_cia, ID, cia_nu_grid, cia_T_grid, dH):
    """
    Calculate emission spectrum using the correlated-k method.
    ...
    tau_cum_w_g = np.zeros((NWAVE,NG))
    tr_old_w_g = np.ones((NWAVE,NG))
    contri_l_w_g = np.zeros((NLAYER,NWAVE,NG))
    for ilayer in range(NLAYER):
        for iwave in range(NWAVE):
            for ig in range(NG):
                tau_cum_w_g[iwave,ig] \
        ...
        for iwave in range(NWAVE):
            for ig in range(NG):
                contri_l_w_g[ilayer,iwave,ig] \
                    = (tr_old_w_g[iwave,ig]-tr_w_g[iwave,ig]) * bb[iwave]
        tr_old_w_g = tr_w_g

    contri_l_w = np.zeros((NLAYER,NWAVE))

    # Integrate over g-ordinates
    for iwave in range(NWAVE):
        for ig in range(NG):
            contri_l_w[:,iwave] += contri_l_w_g[:,iwave,ig] * del_g[ig]

    # Normalise
    for iwave in range(NWAVE):
        contri_l_w[:,iwave] = contri_l_w[:,iwave]/np.sum(contri_l_w[:,iwave])

    # Invert pressure coordinate
    for iwave in range(NWAVE):
        contri_l_w[:,iwave] = contri_l_w[::-1,iwave]
